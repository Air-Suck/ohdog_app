...
2、元操作库仅允许以下两种步骤（与现有 client/ip.ets 下发能力一致）：
    A. mode — 模式切换，对应现有 TCP mode
       {"op":"mode","key":"<k>"}
       合法 key 仅限：r（阻尼）、z（站立）、v（行走）、b（后空翻）、j（跳跃）、k（挥手）
       禁止 key=x（空闲）：该模式会释放关节，存在安全隐患；云端规划不得输出，手机端对规划序列也会整单拒绝
       （手机手动控狗按钮仍可发 x，但不属于元操作库）
...
5、响应中 actions 示例（「向前走约两米再切回阻尼」，按约 0.4m/s × 5s 近似；结束用 r 而非 x）：
    {
      "request_id": "req-001",
      "ok": true,
      "actions": [
